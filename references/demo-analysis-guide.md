...
提取时为每个埋点事件逐字段记录（不要笼统写"采集参数"一坨）:
- 事件 ID（E-01 起）+ 事件名（用 `类型_对象_动作` 下划线小写，前缀 page_ / action_ / event_ 区分类型）
- 所属页面（事件归属的路由页面）
- 上报字段：逐个列出字段名 + 字段类型（string / number / boolean / datetime / object / array）+ 中文含义
- 是否关键字段（标注哪些字段是核心，用于指标计算）
- 取值来源（url / 接口返回 / 本地状态 / performance API，禁笼统写"前端"）
- 上报时机（精确到触发节点，如"校验通过、发起请求前"）
- 关联指标（该事件用于计算哪个核心指标）
