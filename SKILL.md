...
版本号规则（格式 MMDDNNN，共 7 位）:
  MM  = 系统当前日期的月份（2 位，补零）
  DD  = 系统当前日期的日期（2 位，补零）
  NNN = 当天内的交付序号（3 位），从 001 起递增

  ★ 铁律：MM/DD 必须来自系统真实的当前日期（执行 `date +%m%d` 获取）。
    禁止从对话上下文、已有交付文件名、Requirement_log、git 提交时间
    推断日期。即使上下文里出现过旧日期（如 0622），也绝不可沿用，
    必须取今天真实的系统日期。违反此规则会导致版本号串到错误日期。

  计算步骤:
    DATE_CODE = 系统当前日期的 MMDD（如 7月21日 → 0721）
    扫描 PM_Requirement/ 下已有 Requirement_[0-9]{7} 文件夹
    只在前缀 = 今天 DATE_CODE 的目录中，取最大序号
    IF 今天没有任何交付目录:
        DELIVERY_VERSION = DATE_CODE + "001"
    ELSE:
        DELIVERY_VERSION = DATE_CODE + (最大序号 + 1，补足 3 位)
  ...
  ❌ 错误示范（已发生过的真实 bug，必须避免）:
    上下文或 Requirement_log 里出现过 0622001 / 0622002，
    于是新交付沿用旧日期命名为 0622003 ——
    这是错的：它复用了上下文里的旧日期 0622，没有取今天的真实日期。
    正确做法：取系统今天日期，命名为 0721xxx。
...
- 每个交互元素的完整状态（默认/hover/聚焦/点击/loading/禁用/报错/空态）+ 登录态/角色下的可见性差异
...
从产品核心指标出发（每个埋点事件必须逐字段写清，不笼统罗列）:
- 识别三类埋点：页面浏览（每个路由页面）、用户行为（关键交互事件）、业务事件（状态流转终态、核心流程完成节点）
- 每个埋点事件提取六要素：所属页面、上报字段（字段名+类型）、字段中文含义、上报时机、是否关键字段、取值来源
- 字段类型用 string / number / boolean / datetime / object / array；金额类统一存最小货币单位（分）
- 为每个事件分配唯一事件 ID（E-01 起），供第 7.5 节指标计算逻辑引用
...
3. **功能逻辑** — 每个功能：目的 + 操作流程（mermaid）+ 状态机（mermaid）+ **交互规则与元素状态**（默认/hover/聚焦/点击/loading/禁用/报错/空态）+ 校验规则 + 极限场景 + **验收标准（Definition of Done）** + **登录态与可见性差异**
4. **业务规则** — 计算规则 + 条件逻辑 + 数据处理规则
5. **数据模型** — 核心实体 + Mock 数据说明 + **后端与服务能力正式实现要求**（接口能力清单 + 错误场景与错误码 + 性能要求，业务级）
6. **已知缺口** — 简化逻辑 + 缺失功能 + TODO
7. **数据埋点** — 埋点事件总表（事件ID/页面/事件名/类型/上报时机/关键字段）+ 逐事件字段明细（字段名/类型/中文含义/是否关键字段/取值来源）+ 监控指标与计算逻辑（用事件ID+字段表达，放最后）
...
写入规则：
- ★ 更新需求永远新建文档，禁止修改/覆盖历史交付：任何需求调整都写入新的 `PM_Requirement/Requirement_[新版本号]/` 目录，原 requirement.md 与历史版本目录只读、永不改动
...
2. **区分交互状态与视觉细节** — 交互行为状态（默认 / hover / 聚焦 / 点击 / loading / 禁用 / 报错 / 空态 / 登录态差异）是**产品交互逻辑**，必须描述；纯视觉细节（布局结构、组件树、CSS 类名、色值字号）归 UI 文档，不在研发文档写
...
8. **埋点事件逐字段写清** — 每个埋点事件必须写清六要素（所属页面、字段名、字段类型、中文含义、上报时机、是否关键字段），事件统一编号（E-01）；所有事件列完后，再用事件 ID + 字段名写监控指标的计算逻辑
...
18. **版本号日期必须取系统真实值** — 交付版本号 `MMDDNNN` 的 MM/DD 必须来自系统当前日期（执行 `date +%m%d` 获取），禁止从对话上下文、已有文件名、Requirement_log、git 时间推断日期；NNN 为当天交付序号，从 001 起递增（如今天第 2 次交付 → `0721002`）
19. **更新需求 = 新建文档，禁止改原文件** — 任何需求更新都新建 `PM_Requirement/Requirement_[新版本号]/` 目录承载，在新文档里写本次变更；原 requirement.md 与历史版本目录只读、永不修改、永不覆盖，只能被引用为基线
